use crate::docs::models::{DocumentChunk, ExtractedDocument};
use uuid::Uuid;
...
mod tests {
    ...
    fn document_chunker_enriches_context_and_preserves_page_bounds() {
        ...
        let pdf_tokens = (0..1200)
            .map(|index| format!("token{index}"))
            .collect::<Vec<_>>()
            .join(" ");
        let pdf = ExtractedDocument {
            document_id: "doc-pdf".to_string(),
            file_path: "/tmp/docs/guide.pdf".to_string(),
        ...
        let pdf_chunks = chunk_document(&pdf);
        assert_eq!(pdf_chunks.len(), 2);
        assert!(pdf_chunks.iter().all(|chunk| chunk.page_start == Some(7)));
        assert!(pdf_chunks.iter().all(|chunk| chunk.page_end == Some(7)));
        assert!(pdf_chunks.iter().all(|chunk| chunk.kind == "page_part"));
    }
}
