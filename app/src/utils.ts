...
export function statusTone(status: string, busy: boolean): StatusTone {
  ...
  if (/(failed|error|not found|unsupported|wrong|corrupted|incomplete)/i.test(status)) {
    return 'error'
  }
...
export function normalizeQuerySuggestion(value: string) {
  return value.trim()
}
